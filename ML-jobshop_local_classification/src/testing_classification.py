...
def solve_jss_ortools(nmachines=3, njobs=3, seed=1, obj="tmax", h=1.3, level="global", ml_model="DT", trained_ml='', cutoff=100, data_type="raw"):
    ...
    if isinstance(seed, str):
        ...
        for i in range(N):
            ...
            # jtime = [random.randint(1, 10000) for _ in range(nops)]
            totaldur = sum([jtime[j] for j in range(nops)])
            totaltime = np.sum(jtime)
            ...
            job = [(jorder[j],
                    jtime[j],
                    sum([jtime[k] for k in range(j)]),
                    sum([jtime[k] for k in range(j, nops)]),
                    release[i]) for j in range(nops)]
            jobs_data.append(job)
        # print(jobs_data)
    ...
    for job in all_jobs:
        ...
        for task_id, task in enumerate(jobs_data[job]):
            ...
            # start_var = model.NewIntVar(release[job], horizon, 'start_%i_%i' % (job, task_id))
            # start_var = model.NewIntVar(random.randint(0, int(totaltime / 2)), horizon, 'start_%i_%i' % (job, task_id))
            duration = task[1]
    ...
    if ml_model != "NONE" and ml_model != "GP" and ml_model != "LOWEST_MIN" and ml_model != "MIN_DOMAIN_SIZE":
        # compute features
        ...
        # print(var_select_order)

        if level == "pure":
            model.AddDecisionStrategy(var_select_order, cp_model.CHOOSE_FIRST, cp_model.SELECT_MIN_VALUE)
        else:
            model.AddDecisionStrategy(var_select_order, cp_model.CHOOSE_LOWEST_MIN, cp_model.SELECT_MIN_VALUE)

        # model.AddDecisionStrategy(var_select_order, cp_model.CHOOSE_LOWEST_MIN, cp_model.SELECT_MIN_VALUE)
        # model.AddDecisionStrategy(var_select_order, cp_model.CHOOSE_MIN_DOMAIN_SIZE, cp_model.SELECT_MIN_VALUE)
    ...
    elif ml_model == "LOWEST_MIN":
        model.AddDecisionStrategy(varlist, cp_model.CHOOSE_LOWEST_MIN, cp_model.SELECT_MIN_VALUE)
    elif ml_model == "MIN_DOMAIN_SIZE":
        model.AddDecisionStrategy(varlist, cp_model.CHOOSE_MIN_DOMAIN_SIZE, cp_model.SELECT_MIN_VALUE)

    # elif ml_model == "NONE":
    #     # print(varlist)
    #     # model.AddDecisionStrategy(varlist, cp_model.CHOOSE_FIRST, cp_model.SELECT_MIN_VALUE)
    #     model.AddDecisionStrategy(varlist, cp_model.CHOOSE_LOWEST_MIN, cp_model.SELECT_MIN_VALUE)
    #     # model.AddDecisionStrategy(varlist, cp_model.CHOOSE_MIN_DOMAIN_SIZE, cp_model.SELECT_MIN_VALUE)
...
if __name__=='__main__':
    ...
    parser.add_argument('-time_limit', default=3600, type=float, help='cutoff time of CP')
    ...
    # files = ['la01', 'la07']
    trained_ml = []
    if ml_method != "NONE" and ml_method != "GP" and ml_method != "LOWEST_MIN" and ml_method != "MIN_DOMAIN_SIZE":
        trained_ml = joblib.load("../training_data/{}_{}_local_{}.pkl".format(obj, ml_method, data_type))
    if ml_method == "GPc":
    ...
    # for seed in files:
        ...
        if isinstance(seed, str):
            objs, branches = solve_jss_ortools(nmachines=nmachine, njobs=njob, seed=seed, obj=obj, h=h,
                                               level=level, ml_model=ml_method, trained_ml=trained_ml, cutoff=time_limit,
                                               data_type=data_type)
        else:
    ...
    np.savetxt("../results/{}_{}_{}_{}_{}_nm_{}_nj_{}_objs.csv".format(obj, h, ml_method, level, data_type, nmachine, njob), allobjs, delimiter=",")
    np.savetxt("../results/{}_{}_{}_{}_{}_nm_{}_nj_{}_branches.csv".format(obj, h, ml_method, level, data_type, nmachine, njob), allbranches, delimiter=",")
    np.savetxt("../results/{}_{}_{}_{}_{}_nm_{}_nj_{}_runtimes.csv".format(obj, h, ml_method, level, data_type, nmachine, njob), alltime, delimiter=",")
