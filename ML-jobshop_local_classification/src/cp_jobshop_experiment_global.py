...
def solve_jss_ortools(nmachines=3, njobs=3, seed=1, obj="tmax", h=1.3, level="global", ml_model="DT", cutoff=100):
    ...
    while True:
        ...
        if resolve > 0:
            # # set optimal bound
            # model.Add(obj_var <= optimal_bound)

            # ordering decision variables based on the optimal solution
            var_priorities = {}
            for var in list_of_vars:
                (job, task_id) = var[2]
                var_priorities[all_tasks[job, task_id].start] = 0
            for var1 in list_of_vars:
                ...
                for var2 in list_of_vars:
                    ...
                    count2 = var2[3]
                    if count1 < count2:
                        if solver.Value(all_tasks[job1, task_id1].start) < solver.Value(all_tasks[job2, task_id2].start):
                            var_priorities[all_tasks[job1,task_id1].start] += 1
                        elif solver.Value(all_tasks[job1, task_id1].start) > solver.Value(all_tasks[job2, task_id2].start):
                            var_priorities[all_tasks[job2,task_id2].start] += 1
        ...
        optimal_bound = solver.Value(obj_var)
...
if __name__=='__main__':
    ...
    for mode in ["order"]:
        ...
        np.savetxt("../results/{}_optimal_global_nm_{}_nj_{}_branches.csv".format(obj, nmachine, njob), allbranches, delimiter=",")
        np.savetxt("../results/{}_optimal_global_nm_{}_nj_{}_runtimes.csv".format(obj, nmachine, njob), allruntimes, delimiter=",")
        np.savetxt("../results/{}_optimal_global_nm_{}_nj_{}_objs.csv".format(obj, nmachine, njob), allobjs, delimiter=",")
