    ...
    if status == cp_model.OPTIMAL:
        ...
        for var in list_of_vars:
            ...
            disjunctive_raw_features.append([
                float(task_id),
                float(jobs_data[job][task_id][1]),
                float(jobs_data[job][task_id][2]),
                float(jobs_data[job][task_id][3]),
                float(totaljobduration[job]),
                float(duedate[job]),
                float(weight[job]),
                float(release[job]),
                float(feature_dict[all_tasks[job, task_id].start]["earliest_start"]),
                float(feature_dict[all_tasks[job, task_id].start]["workload"])
            ])
            labels.append(float(solver.Value(all_tasks[job, task_id].start)))
            ...
            # ])
            # labels.append(float(solver.Value(all_tasks[job, task_id].start)) / max_totaljobduration)
...
if __name__=='__main__':
    ...
    df_raw.to_csv("../training_data/training_data_{}_regression_nm_{}_nj_{}_h{}.csv".format(obj, nmachine, njob, h), index=False)
