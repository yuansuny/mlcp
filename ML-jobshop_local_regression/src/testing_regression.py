    ...
    if ml_model != "NONE" and ml_model != "GP":
        ...
        for var in list_of_vars:
            ...
            disjunctive_raw_features.append([
                float(task_id) / nops,
                float(jobs_data[job][task_id][1]) / max_totaljobduration,
                float(jobs_data[job][task_id][2]) / max_totaljobduration,
                float(jobs_data[job][task_id][3]) / max_totaljobduration,
                float(totaljobduration[job]) / max_totaljobduration,
                float(duedate[job]) / max_totaljobduration,
                # float(duedate[job]) / max_duedate,
                float(weight[job]) / max_weight,
                float(release[job]) / max_totaljobduration,
                float(feature_dict[all_tasks[job, task_id].start]["earliest_start"]) / max_totaljobduration,
                float(feature_dict[all_tasks[job, task_id].start]["workload"]) / feature_dict[all_tasks[job, task_id].start]["maxload"],
            ])
        ...
        for var in list_of_vars:
            (job, task_id) = var[2]
            var_priorities[all_tasks[job,task_id].start] = predicted_label[count]
            count += 1
...
if __name__=='__main__':
    ...
    # for seed in files:
        ...
        if isinstance(seed, str):
            objs, branches = solve_jss_ortools(nmachines=nmachine, njobs=njob, seed=seed, obj=obj, h=h, level=level,
                                ml_model=ml_method, trained_ml=trained_ml, cutoff=time_limit, data_type=data_type)
        else:
